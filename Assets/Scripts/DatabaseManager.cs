...
public class DatabaseManager : MonoBehaviour
{
    ...
    public void Save()
    {
        ...
        for (int i = 0; i < GameManager.gm.characterLevel.Length; i++)
        {
            playerData.charLevels[i] = GameManager.gm.characterLevel[i];
        }
        for (int i = 0; i < GameManager.gm.real_estate_havable.Length; i++)
        {
            playerData.realEstate[i] = GameManager.gm.real_estate_havable[i];
        }
        for (int i = 0; i < GameManager.gm.Franchisee_havable.Length; i++)
        {
            playerData.frenchaisee[i] = GameManager.gm.Franchisee_havable[i];
        }
